...
class JmsMegaMenu extends Module
{
    ...
    public function install()
    {
        if (parent::install() && $this->registerHook('header') && $this->registerHook('displayTopColumn') && $this->installSamples()) {
            ...
            $id_tab1 = $this->addTab('Jms MegaMenu', 'dashboard', $tab_parent_id, 3, 'menu');
    ...
    private function addTab($title, $class_sfx = '', $parent_id = 0, $position = 0, $icon = '')
    {
        if ($parent_id > 0) {
    ...
    public function mobileGenItem($itemid)
    {
        ...
        if (!in_array($itemid, $this->mobile_gens)) {
            ...
            if ((!isset($setting['title']) || ((int)$setting['title']==1)) && $item['type'] != 'theme-logo') {
                ...
                if (($item['level'] == 0) && isset($this->children[$itemid])) {
                    $this->mobilemenu .= '<em class="fa fa fa-plus"></em>';
                }
                $this->mobilemenu .= '</a>';
            }
