...
<div class="jms-slider-wrapper">
	<div class="responisve-container">
		...
		{foreach from=$slider->slides item=slide}
			<div class="slide {$slide->class_suffix}" style="background:{$slide->bg_color} url({$root_url nofilter}modules/jmsslider/views/img/slides/{$slide->bg_image nofilter}) no-repeat left top;background-size:cover;" {if $slide->slide_link}onclick="document.location='{$slide->slide_link nofilter}';"{/if}>
				{foreach from=$slide->layers item=layer}
					...
					{if $layer->desktop->data_line_height}
					data-lineheight = "{$layer->desktop->data_line_height nofilter}px"
					{/if}
